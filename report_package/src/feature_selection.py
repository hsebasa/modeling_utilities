...
from standard_modeling.utilities import gen_scorer
...
def cross_validate_model(model, X, y, cv=5, scoring=None, fit_kwargs=None, score_kwargs=None, sample_weights=None, random_state=None):
    """
    ...
    for train_index, test_index in cv.split(X):
        ...
        model_clone.fit(X_train, y_train, **fit_kwargs_with_weights)
        score = scorer(model_clone, X_test, y_test, **score_kwargs)
        scores.append(score)
    return scores
...
class FeatureSelector:
    def __init__(self, cv=5, scoring=None, vif_threshold=5, score_threshold=0.01,
